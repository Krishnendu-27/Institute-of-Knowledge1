...
const CourseDetails = ({ courseId, onBack }) => {
  const getClassById = useClassStore((state) => state.getClassById);
  const getStudentProgress = useClassStore((state) => state.getStudentProgress);
  const removeStudentInClass = useClassStore((state) => state.removeStudentInClass);
  const updateClass = useClassStore((state) => state.updateClass); // Added store function for updating class
  const isLoading = useClassStore((state) => state.isLoading);
  ...
  const handleOpenEditModal = () => {
    ...
    setEditFormData({
      name: mainClass?.name || "",
      startDate: mainClass?.startDate ? new Date(mainClass.startDate).toISOString().split("T")[0] : "",
      duration: mainClass?.duration || "",
      endDate: mainClass?.endDate ? new Date(mainClass.endDate).toISOString().split("T")[0] : "",
      fees: mainClass?.fees || "",
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      // Assuming updateClass takes the class ID and the updated data payload
      await updateClass(courseData.mainClass._id, editFormData);
      
      // Update local state to reflect changes instantly without refreshing
      setCourseData((prevData) => ({
        ...prevData,
        mainClass: {
          ...prevData.mainClass,
      ...
      toast.error(error.response?.data?.message || "Failed to update course details.");
    } finally {
              ...
              <div className="p-6 overflow-y-auto flex-1">
                <form id="editCourseForm" onSubmit={handleEditSubmit} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      <label className="text-sm font-medium text-slate-700">Instructor Email</label>
                      <input
                        type="email"
                        name="teacherEmail"
                        required
